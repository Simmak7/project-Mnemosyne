...
import { useDocCollections } from '../hooks/useDocumentCollections';
...
function DocumentLayout({ onNavigateToNote, selectedDocumentId, onClearSelection }) {
  ...
  // Compute status counts from the full (unfiltered by status) document set
  ...
  }, [selectedDocumentId, onClearSelection]);

  const handleDocSelect = useCallback((docId) => {
    setSelectedDocId(docId);
    const doc = documents.find(d => d.id === docId);
    if (doc?.ai_analysis_status === 'needs_review') {
      setViewMode('review');
    }
  }, [documents]);
  ...
  const groupedDocs = useMemo(() => {
    ...
    for (const doc of filteredDocs) {
      let key;
      if (groupBy === 'status') {
        key = doc.ai_analysis_status || 'pending';
      } else if (groupBy === 'document_type') {
        key = doc.document_type || 'Unknown';
      } else {
        key = 'All';
      }
            ...
            <div className="document-collections-section">
              <div className="collections-section-header-row">
                ...
                >
                  ...
                  {collections.length > 0 && (
                    <span className="collections-header-count">{collections.length}</span>
                  )}
                </button>
                <button
              ...
              {collectionsExpanded && (
                <div className="collections-list">
                  {collections.length > 0 && (
                    <button
                      className={`collection-item ${!selectedCollectionId ? 'active' : ''}`}
                      onClick={() => setSelectedCollectionId(null)}
                    >
                      <FolderOpen size={14} />
                      <span className="collection-item-name">All Documents</span>
                    </button>
                  )}
                  {collections.map(c => (
                    <button
                      key={c.id}
                      className={`collection-item ${selectedCollectionId === c.id ? 'active' : ''}`}
                  ...
                  {showNewCollectionForm && (
                    <form className="collection-create-form" onSubmit={handleCreateCollection}>
                      ...
                      <button
                        type="submit"
                        className="collection-create-btn confirm"
                      ...
                      <button
                        type="button"
                        className="collection-create-btn cancel"
                        onClick={() => {
                          setShowNewCollectionForm(false);
                          setNewCollectionName('');
                        }}
