    ...
    try:
        ...
        try:
            if body.conversation_id:
                ...
                # If conversation not found (wrong owner or doesn't exist), reset to allow auto-create
                if not conversation:
                    logger.warning(f"Conversation {body.conversation_id} not found for user {current_user.id}, will auto-create")
                    conversation_id = None

            # Auto-create conversation if needed (either no ID provided or ID not found)
            if not conversation and body.auto_create_conversation:
                # Auto-create new conversation with smart title
                ...
                logger.info(f"Auto-created conversation {conversation_id}: '{title}'")
...
async def rag_query_stream(
    ...
    4. Save to conversation (auto-create if needed)
    ...
    **Rate limit:** 20 requests/minute
    ...
    # Handle conversation creation/lookup before streaming
    conversation_id = body.conversation_id
    conversation = None
    ...
    if body.conversation_id:
        ...
        # If conversation not found (wrong owner or doesn't exist), reset to allow auto-create
        if not conversation:
            logger.warning(f"Conversation {body.conversation_id} not found for user {user_id}, will auto-create")
            conversation_id = None
        ...
        try:
            ...
            if conversation:
                try:
                    ...
                    logger.error(f"Failed to save streaming conversation: {save_err}")
                    db.rollback()

            # Send metadata with conversation_id
            metadata = {
                'total_sources_searched': retrieval_summary['total_sources_searched'],
                'sources_used': len(assembled_context.sources),
                'retrieval_methods_used': retrieval_summary['retrieval_methods_used'],
    ...
    **Rate limit:** 30 requests/minute
    ...
    for msg in messages:
        ...
        if msg.role == "assistant":
            ...
            for c in db_citations:
                ...
                citations.append(schemas.CitationSource(
                    ...
                    retrieval_method=c.retrieval_method,
                    hop_count=c.hop_count or 0,
                    relationship_chain=None  # Not stored in DB
                ))
        ...
        message_responses.append(schemas.MessageResponse(
            ...
            retrieval_metadata=None,  # Field not in model, metadata stored elsewhere
            citations=citations
        ))
